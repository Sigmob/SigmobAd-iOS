...
(function() {
    ...
    sigmob.loadProduct = function(kwargs) {
        if (!kwargs) {
    ...
    sigmob.dissStoreKit = function(mode) {
        if (!mode) {
    ...
    sigmob.addEventListener = function(event, listener) {
        if (!event || !listener) {
            broadcastEvent(EVENTS.ERROR, 'Both event and listener are required.', 'addEventListener')
        } else if (!contains(event, EVENTS)) {
            broadcastEvent(EVENTS.ERROR, 'Unknown SIGMOB event: ' + event, 'addEventListener')
        } else {
            if (!listeners[event]) {
    ...
    sigmob.removeEventListener = function(event, listener) {
        if (!event) {
    ...
    sigmob.notifyStorekitReadyEvent = function(itunesId) {
        broadcastEvent(EVENTS.STOREKIT_READY, itunesId)
    };
    sigmob.notifyStorekitDidFailToLoadEvent = function(itunesId, error) {
        broadcastEvent(EVENTS.STOREKIT_DIDFAILTOLOAD, itunesId, error)
    };
    sigmob.notifyStorekitClickEvent = function(itunesId) {
        broadcastEvent(EVENTS.STOREKIT_CLICK, itunesId)
    };
    sigmob.notifyStorekitPresentEvent = function(itunesId) {
        broadcastEvent(EVENTS.STOREKIT_PRESENT, itunesId)
    };
    sigmob.notifyStorekitFinishEvent = function(itunesId) {
        broadcastEvent(EVENTS.STOREKIT_FINISH, itunesId)
    };
    sigmob.notifyOverlayDidFailToLoadEvent = function(itunesId, error) {
        broadcastEvent(EVENTS.OVERLAY_DIDFAILTOLOAD, itunesId, error)
    };
    sigmob.notifyOverlayPresentEvent = function(itunesId) {
        broadcastEvent(EVENTS.OVERLAY_DIDPRESENT, itunesId)
    };
    sigmob.notifyOverlayClickEvent = function(itunesId) {
        broadcastEvent(EVENTS.OVERLAY_CLICK, itunesId)
    };
    sigmob.notifyOverlayFinishEvent = function(itunesId) {
